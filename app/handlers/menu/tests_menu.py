...
async def finished_tests_handler(call: types.CallbackQuery):
    ...
    await call.message.edit_text(finished_tests, reply_markup=MenuKb().back_to_menu(i18n))
...
async def show_test_handler(call: types.CallbackQuery, callback_data: Dict):
    ...
    await call.message.edit_text(test.preview, reply_markup=MenuKb().start_test(test_id, i18n))
...
async def get_available_tests(db: Database, user_id: int):
    ...
    if tests:
        ...
        message.append(i18n("Вы прошли все тесты!"))
    keyboard.row(types.InlineKeyboardButton(i18n("Вернуться назад"),
...
async def get_finished_tests(db: Database, user_id: int):
    ...
    for test_num, score in enumerate(scores):
        sub_msg = [i18n("<b>Тест #{test_num}</b>").format(test_num=test_num + 1),
                   i18n("Поздравляем! "
